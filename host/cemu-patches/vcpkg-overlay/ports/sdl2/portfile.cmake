vcpkg_from_github(
    ...
    PATCHES
        deps.patch
        alsa-dep-fix.patch
        # DualDeck (github.com/crimson3076/DualDeck): real Fedora build
        # failure, 2026-08-26 -- src/audio/pipewire/SDL_pipewire.c:593
        # passes node->proxy (struct pw_proxy*) directly to
        # pw_node_enum_params(), which current PipeWire headers declare
        # as taking struct pw_node* -- a strict-typing tightening in
        # PipeWire itself since this SDL release was cut, not a bug in
        # SDL's actual logic (the two types share pw_proxy's
        # spa_interface layout by design; this is the documented way to
        # call a typed method through a bound proxy). Vcpkg builds SDL2
        # from source against whatever PipeWire dev headers are on the
        # build machine, so this only surfaces on a distro whose
        # pipewire-devel is newer than whatever the CI runner that
        # validated this SDL release had -- Fedora hit it; ubuntu-latest
        # (this project's own release.yml runner) has not, as of this
        # writing. See this port's own pipewire-node-proxy-cast.patch
        # for the one-line fix (an explicit cast at the call site) and
        # host/cemu-patches/vcpkg-overlay/README.md for why this whole
        # overlay port exists instead of patching vcpkg's own checkout
        # in place.
        pipewire-node-proxy-cast.patch
)
