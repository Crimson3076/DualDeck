...
// alongside the existing JPEG one. Purely additive in *behavior*: every
// real client today only advertises VideoCodec_Jpeg, and the host only
// ever selects VideoCodec::Jpeg back (no encoder for anything else exists
// yet) -- but it's still a genuine payload-shape change (HelloPayload
// gains a trailing byte, HelloAckPayload gains one after mode), so an
// old/new version mismatch on either side still needs the usual
// kProtocolVersion bump rather than silent misparsing.
//
...
// either side.
// Video codec capability negotiation (protocol v13). JPEG (via
// libjpeg-turbo) has been the only video format since v8 and remains the
// permanent, dependency-free baseline every build supports -- these
// bits/values exist so a future codec (H.264, first candidate) can be
// added as something both sides opt into, not a hard replacement: an
...
struct HelloPayload {
    ...
    // Bitmask of VideoCodecBit_* values (protocol v13) the client can
    // decode. Every real client sets at least VideoCodecBit_Jpeg --
    // JPEG decode has no external dependency and has been the only
    // format since v8, so it's the one thing every build can always do.
    // VideoCodecBit_H264 is set only by a client built with H.264 decode
    // support; a client from before v13 (or an older wire version
    // rejected before this is even parsed) is equivalent to advertising
...
struct HelloAckPayload {
    ...
    // Which VideoCodec this session's VideoFrame packets are actually
    // encoded with (protocol v13), chosen by the host by intersecting
    // this against HelloPayload::supportedVideoCodecs -- see that
    // field's own comment. Sent regardless of accepted/rejectReason,
    // same convention as system/adapter/mode above. Always
    // VideoCodec::Jpeg today (no other encoder exists yet); the field
...
inline constexpr size_t kControllerStateWireSize =
    4 + 8 + 2 + 2 + 2 + 2 + 2 + 2 + 1 + 2 + 2 + 2 + 2 + 1 + 1 + 1 + 1;
