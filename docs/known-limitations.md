...
## 2026-08-03: Wayland screen-mirror still failing after the SPA-plugin-path fix -- PIPEWIRE_MODULE_DIR needed the same treatment, and the original detection logic had a real bug of its own

The fix above got the user past `pw_loop_new()` -- confirmed by a second
round of real `PIPEWIRE_DEBUG=3` output, `SPA_PLUGIN_DIR` now resolves
correctly -- but `pw_context_new()` then failed on a different mandatory
module:

```
could not load mandatory module "libpipewire-module-protocol-native": No such file or directory
```

Same root shape (a `dlopen()`'d PipeWire component with its own baked-in,
CI-Ubuntu-specific search path -- `PIPEWIRE_MODULE_DIR` this time instead
of `SPA_PLUGIN_DIR`), but the fact that the *identical* detection
technique fixed one variable and not the other pointed at a real bug in
`pipewire_env.sh` itself, not just an incomplete candidate-path list.
Rather than guess again, this was investigated with a Plan agent
dispatched specifically to read PipeWire's own upstream C source
(`pipewire.c`, `impl-module.c`) before proposing a fix, to avoid a fourth
guess-and-fail round.

**Real root cause, confirmed from PipeWire's own source:** PipeWire's
`getenv()`-based fallback to its own correct compiled-in default only
triggers when the variable is *completely unset* (`getenv() == NULL`). If
it's set to anything else -- including a wrong path -- PipeWire trusts
that value and never falls back, even though it's broken. The original
`pipewire_env.sh` guarded each variable with `[[ -z "${VAR:-}" ]]` --
"only detect if it isn't already set" -- and only checked `-d` (directory
exists), not whether the actual needed `.so` was inside it. If Bazzite's
own ambient environment (systemd `--user`'s imported environment,
`/etc/environment`, or similar) already exported `PIPEWIRE_MODULE_DIR` to
something that doesn't contain the module, the original logic would
silently never correct it -- while `SPA_PLUGIN_DIR`, evidently unset in
that same ambient environment, sailed through the exact same code path
untouched. This is why identical logic produced different outcomes for
the two variables.

**Fix:** rewrote `scripts/lib/pipewire_env.sh`'s
`find_and_export_pipewire_dirs()` around a shared `_pw_resolve_dir()`
helper, called once per variable, with a real three-tier resolution
order:

1. **Validate an existing value** against the actual target file it needs
   to contain (`_pw_dir_has_file`) -- not just non-empty, not just a
   directory. A valid override is left alone; an invalid one is logged to
   stderr and discarded instead of trusted.
2. **pkg-config** (`_pw_pkgconfig_dir`) -- queries the host's own real
   installed PipeWire directly (`pkg-config --variable=plugindir
   libspa-0.2`, `--variable=moduledir libpipewire-0.3`), generated by
   PipeWire's own meson build into its `.pc` files -- more authoritative
   than guessing a distro convention. Guarded by `command -v pkg-config`
   so a minimal/atomic image without it (a real possibility on Bazzite)
   falls through cleanly, no error.
3. **The original hardcoded candidate-path scan**, unchanged, but now
   validated against the real target file via the same helper as the
   other two tiers instead of a bare `-d` check.

On total failure, the variable is `unset` rather than left holding an
empty string -- exporting `""` would be strictly worse than leaving it
unset, per the same `getenv() == NULL` semantics above. Every branch
(discarded-bad-override, resolved-via-pkg-config, resolved-via-candidate-
scan, gave-up-and-unset) logs a one-line message to stderr, so the next
diagnostic round is self-explanatory regardless of outcome, rather than
another silent failure needing a fifth guess.

**Verified:** `bash -n` clean on the rewritten `pipewire_env.sh` and on
`build-release.sh` (confirmed unchanged, both call sites --
`host-control-daemon.sh` and `run-host.sh`'s manual Host-Control branch --
still just `source ./pipewire_env.sh; find_and_export_pipewire_dirs` with
no arguments, matching the rewrite's preserved external interface).
Directly sourced the rewritten script in this sandbox (Debian/Ubuntu-
based, has real pkg-config and `.pc` files) and confirmed normal
resolution via the pkg-config tier for both variables. Simulated all
three non-trivial branches by hand: a bogus pre-set `PIPEWIRE_MODULE_DIR`
override is correctly discarded and re-resolved; hiding `pkg-config` from
`PATH` correctly falls through to the candidate-scan tier for both
variables; a variable with no valid override, no pkg-config match, and no
matching candidate correctly ends up `unset` with the expected stderr
message. Full local `ctest` run (6 suites) passes with no regressions.

**Not yet verified:** on the actual affected Bazzite machine -- whether
this actually resolves `PIPEWIRE_MODULE_DIR` correctly there (via
pkg-config, the candidate scan, or reveals a still-different problem via
one of the new stderr diagnostic lines), and whether screen mirroring
...
## Host/client menu no longer exits after every action, plus an Installation branch selector (GitHub issue, 2026-08-27/29)

**The bug**: real user report, "Check for updates / changing configuration
/ applying or saving settings all close DualDeck Host instead of returning
to the menu." Root cause: `host/dualdeck-host.sh` and
`client/dualdeck-client.sh` (generated by heredocs in
`scripts/build-release.sh`) called their menu-picker function exactly
once and then fell off the end of the script -- there was no loop back to
the menu. `launch`/`emudeck` deliberately `exec` into another process
(correct, unchanged), but every other action (Check for updates, Add to
Steam, Remove from Steam, Reconfigure Controls, the Host Control daemon
submenu, the client's trackpad-experiment toggle) silently ended the
whole process once it finished -- success or failure alike. Backing out
of a submenu (`choose_emulator()`'s and the daemon submenu's own "Back")
had the identical bug in a sharper form: it hit an explicit `exit 0`,
so even *cancelling* a dialog closed the host.

**The fix**: both generated scripts now wrap their action dispatch in a
`while true; do ... done` loop. Only the top-level menu's own explicit
Cancel/Exit breaks out of it; every other action (completed or failed --
each `internal/` script already reports its own error and returns
non-zero rather than aborting the whole menu) and every submenu "Back"
falls through to the bottom of the loop and redisplays the same menu.
Regression-tested in `tests/host_client_menu_lifecycle_test.py` (see
`docs/testing.md`), which extracts both heredocs and drives them via
scripted stdin against stub `internal/` scripts -- covering a completed
action, a failed one, and cancelling a submenu, asserting the process is
still alive and shows the menu again in every case.

**Installation branch selector, added alongside this fix**: both menus
gained an "Advanced..." entry with `Installation branch: <name> (installed:
<short-sha>, <tag>)`, `Change installation branch...`, `Refresh branch
list`, and `Install selected branch` -- backed by a new shared
`scripts/lib/dualdeck_branch.sh` (bundled identically into both
`host/internal/` and `client/internal/`, one implementation rather than
two hand-written copies) plus `host/internal/install-branch.sh` and
`client/internal/install-branch.sh`. Branches are discovered from GitHub
with real pagination and cached (1-hour TTL, explicit Refresh bypasses
it); selecting a branch only ever updates local state, never triggers an
install. "Install selected branch" resolves the branch to the newest
published release built exactly at that branch's current tip commit,
downloads and checksum-verifies that specific release (not "latest"),
and only then hands off to the existing already-atomic
`install-steam-shortcut.sh --force` stage-then-swap (`.new` staging
directory, one generation kept as `.previous`) -- nothing is ever
recorded as installed unless the whole sequence actually completes, so a
bad checksum or a network failure leaves the previous install completely
untouched. An existing install with no prior branch selection is labeled
with the repository's default branch and nothing is written to disk just
by opening the menu -- no implied reinstall for existing users.

**A real constraint worth being explicit about**: "install from any
branch" is bounded by what's actually publishable today.
`release.yml` is deliberately manual (`workflow_dispatch` only, not
triggered by every push -- see that workflow's own header comment on
why: each build is 20-90+ minutes of melonDS/Azahar/Cemu/SDL3
compilation), so most branches other than the default one have **no**
release built from their current tip at any given moment. Selecting such
a branch and choosing "Install selected branch" fails cleanly with "no
DualDeck release has been published from branch '&lt;name&gt;''s current
commit yet" -- it does not build anything on the fly, and it never
silently substitutes `main`. This is a deliberate scope choice (see the
three options weighed for this feature: reusing existing manual
releases vs. a new build-on-every-push CI pipeline vs. building from
source locally on install) -- auto-building every push would directly
contradict `release.yml`'s own reasoning for being manual, and building
from source locally isn't practical on a Steam Deck client. A branch
becomes installable the moment someone runs the Release workflow against
it.

Host and client are always separate machines, so "the same branch and
resolved commit for both" is not a live cross-machine transaction --
each side independently resolves the identical branch name against the
same GitHub repository and installs from the identical release archive
that name resolves to, and each side displays its own resolved commit so
a mismatch (e.g. installing host and client minutes apart, straddling a
new release) is visible rather than silent.

**Verified**: `bash -n`/`shellcheck` on both generated scripts and the
new library/install scripts; the full existing test suite (`ctest`,
`tests/smoke_test.py`, `tests/device_approval_smoke_test.py`,
`scripts/check-patch-protocol-sync.sh`) still passes unchanged;
`tests/host_client_menu_lifecycle_test.py` and
`tests/dualdeck_branch_selector_test.py` (new, see `docs/testing.md`)
cover both features end-to-end, including manually walking the real
generated menu through a terminal session (Check for updates, a
simulated failed action, backing out of both submenus, and a full
